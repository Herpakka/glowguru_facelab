...
import numpy as np
from helper import resize_with_aspect_ratio, posterize, draw_landmarks_Tzone, find_contours, naming_code,use_clahe
    ...
    try:
        ...
        with mp_face_mesh.FaceMesh(
            ...
            if results.multi_face_landmarks:
                annotated_image = draw_landmarks_Tzone(rgb_image, results.multi_face_landmarks,mode="Tzone")
                fore_img = draw_landmarks_Tzone(rgb_image, results.multi_face_landmarks,mode="fore")
                chk_img = draw_landmarks_Tzone(rgb_image, results.multi_face_landmarks,mode="chk")
                ...
                _, fore_img = cv2.threshold(fore_img, 30, 255, cv2.THRESH_BINARY)
                _, chk_img = cv2.threshold(chk_img, 30, 255, cv2.THRESH_BINARY)
                
                
                contours = find_contours(final_img2)
                contours_fore = find_contours(fore_img)
